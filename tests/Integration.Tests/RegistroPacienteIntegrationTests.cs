...
public class RegistroPacienteIntegrationTests : IDisposable
{
    ...
    [Fact]
    public async Task Registrar_PacienteValido_PersistePacienteEnBD()
    {
        // Arrange
        var paciente = CrearPacienteValido("12345678");
    ...
    [Fact]
    public async Task Registrar_DocumentoDuplicado_RechazaYNoPersiste()
    {
        // Arrange
        var primero   = CrearPacienteValido("12345678");
        var duplicado = CrearPacienteValido("12345678");
        await _registro.Registrar(primero);

        // Act
        var resultado = await _registro.Registrar(duplicado);
    ...
    [Fact]
    public async Task Registrar_EdadInvalida_NoPersiste()
    {
        // Arrange
        var paciente = CrearPacienteValido("99999999") with { Edad = 150 };
    ...
    // Then:  la BD contiene exactamente 2 pacientes y ObtenerPacientes los devuelve
    [Fact]
    public async Task Registrar_DosPacientesDistintos_PersisteLosDoS()
    {
        // Arrange
        ...
        // Act
        var r1 = await _registro.Registrar(paciente1);
        var r2 = await _registro.Registrar(paciente2);

        // Assert
        r1.Should().Be(ResultadoRegistro.Exitoso);
        r2.Should().Be(ResultadoRegistro.Exitoso);
        _context.Pacientes.Should().HaveCount(2);
        var pacientes = await _registro.ObtenerPacientes();
        pacientes.Should().HaveCount(2);
    }
